...
public:
	...
	void UpdateTexture() {
		...
		if (_ScreenBuffer == nullptr || _ScreenBuffer->Width != displayWidth ||
			_ScreenBuffer->Height != displayHeight) {
		...
		*/
		...
		if (_PboSupported == true) {
			if (resizeTexture == true) {
				...
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, displayWidth, displayHeight, 0, GL_BGRA_EXT,
			...
			glBindTexture(GL_TEXTURE_2D, _ScreenTexture);
			glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, displayWidth, displayHeight, GL_BGRA_EXT,
							GL_UNSIGNED_INT_8_8_8_8_REV, nullptr);
			glBindTexture(GL_TEXTURE_2D, 0);
		...
		// TexImage2D method
			...
			// glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, displayWidth, displayHeight, GL_BGRA_EXT,
			// GL_UNSIGNED_INT_8_8_8_8_REV, (GLvoid*)_ScreenBuffer->Pixels);
			glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, displayWidth, displayHeight, GL_BGRA_EXT,
							GL_UNSIGNED_INT_8_8_8_8_REV, (GLvoid*)displayScreen->GetPixels());
