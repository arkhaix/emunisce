...
KingsDream::KingsDream() {
	...
	m_skipRanges.push_back(std::make_pair(
		-0.72f, 0.f));  ///< Bad place.  This range murders the cpu.  Probably throws bad values into sin().
	m_skipRanges.push_back(std::make_pair(0.f, 1.72f));  ///< Boring section
	// m_skipRanges.push_back( std::make_pair(0.7f, 1.28f) );	///<Blank section	///<Above "boring section"
	// encompasses this
...
void KingsDream::ResizeScreenBuffers(unsigned int width, unsigned int height) {
	if (m_screenBuffer != nullptr && m_screenBuffer->GetWidth() == (int)width &&
		m_screenBuffer->GetHeight() == (int)height) {
		return;
	}
