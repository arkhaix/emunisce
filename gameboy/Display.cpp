...
void Display::SetLcdStatus(u8 value) {
	...
	CheckCoincidence();  ///< Enabling or disabling this seems to have no impact?  Probably because writing
						 ///< STAT_Coincidence can only clear it?
...
void Display::SetCurrentScanline(u8 value) {
	...
	// CheckCoincidence();	///<Enabling or disabling this seems to have no impact?  Why doesn't it break the
	// prehistorik man title?
}
...
void Display::Begin_VBlank() {
	...
	CheckCoincidence();  ///< Disabling this has no impact?  IF_VBLANK is higher priority... but some games might still
						 ///< rely on LCDC LYC=LY only?
...
void Display::Begin_SpritesLocked() {
	...
	m_currentScanline++;
	if (m_currentScanline >=
		144) {  ///< After a VBlank, this will be 153.  If we've reached this point, then we've started a new frame.
		m_currentScanline = 0;
	}
...
void Display::RenderSpritePixel(int screenX, int screenY) {
	...
	// RenderSprites fills m_frameSpriteData.  If there's no value there at this pixel, then there's no sprite at this
	// pixel.
...
void Display::RenderSprites(int screenY) {
	...
	for (int i = 0; i < 40; i++) {
		...
		int cacheScreenY = screenY;  ///< todo: This is leftover from when full tiles were cached (as opposed to only
									 ///< the current line).  It's safe to remove this.
...
void Display::UpdateTileData(int bank, u16 address, u8 data) {
	...
	if (baseVramAddress &
		1) {  ///< Bytes 0 and 1 of the line will always start on an even boundary.  If baseVramAddress is odd, then
			  ///< we're modifying the high byte and the low byte is baseVramAddress-1.
		lineAddress--;
	}
